...
A list of setup commands to run in the environment after your system packages and Python packages have been installed. If you're familiar with Docker, it's like a `COPY` instruction in your `Dockerfile`. The scope of the filesystem will be limited to `./src` by `WORKDIR ./src` in the `Dockerfile` generated by cog.
...
build:
  copy:
    - ~/copy-data-source /copy-data-target
    - ../other-data-source.txt /folder/other-data-target.txt
```

Cog still adds `COPY ./src` to the `Dockerfile` as the last step to copy current directories contents to the image
